...
def f_theta_multiple_images(ddim_model, x_t, t, z_sem, max_t, beta_small, beta_large):
    ...
    e_theta = estimate_noise(ddim_model, noised_images, time_steps, z_sem)
    
    for i in range(t.shape[0]):
        _alpha_bar_t = _alpha_bar(int(t[i]), max_t, beta_small, beta_large)
        _sqrt_alpha_bar_t = math.sqrt(_alpha_bar_t)
        _sqrt_one_minus_alpha_bar_t = math.sqrt(1 - _alpha_bar_t)
        result[i] = (1.0 / _sqrt_alpha_bar_t) * (x_t - (_sqrt_alpha_bar_t * e_theta[i]))
    return result
    
#You can use this for one image by unsqueezing/creating the batch dimension for x_t and z_sem
def diffusion_denoise_process_multiple_images(ddim_model, x_t, z_sem, max_t, beta_small, beta_large):
    ...
    for i in range(time_steps.shape[0]):
        ...
        if current_t > 0:
            x_t = q_distribution_multiple_images(x_t, f_theta_result, max_t, beta_small, beta_large)
        elif current_t == 0:
            x_t = f_theta_result
    return x_t

#You can use this for one image by unsqueezing/creating the batch dimension for x_t and z_sem    
def diffusion_stochastic_encode_process_multiple_images(ddim_model, x_0, z_sem, max_t, beta_small, beta_large):
